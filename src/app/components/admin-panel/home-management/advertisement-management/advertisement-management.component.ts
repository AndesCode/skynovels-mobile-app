...
export class AdvertisementManagementComponent implements OnInit {
  ...
  save(advertisementForm: NgForm) {
    if (this.uploading || advertisementForm.invalid || (!advertisementForm.dirty && !this.fileToUpload)) {
      this.ps.presentToast('danger', 'Formulario invalido');
      return;
    }
    this.uploading = true;
    ...
    request.subscribe((resp: any) => {
      ...
      if ( this.fileToUpload ) {
        ...
        .then((img: any) => {
          ...
          this.fileToUpload = null;
          this.ps.presentToast('success', '¡Cambios guardados!');
          this.uploading = false;
          return;
        }).catch(error => {
          this.ps.presentToast('danger', error.message);
        });
      } else {
        this.uploading = false;
        this.ps.presentToast('success', '¡Cambios guardados!');
  ...
  async deleteAdvertisementConfirm() {
